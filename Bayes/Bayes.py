...
def dataload():
    ...
    for i in range(1, 26):# 垃圾与非垃圾邮件各25封
...
def random_split():
    ...
    testSet = []  # 测试集的下标列表
    for i in range(15):  # 取35个作为训练集，15个作为测试集
...
def trainNB0(trainMat, trainClasses):# trainMat：训练文档矩阵；trainClasses：训练类别标签矩阵
    ...
    pAbusive = sum(trainClasses) / float(numTrainDocs)# 得到垃圾邮件先验概率
    # 单词出现次数初始化为1，避免出现0
    ...
    for i in range(numTrainDocs):
        if trainClasses[i] == 1:
            p1Num += trainMat[i]# 矩阵相加，得到的矩阵表示单词出现情况
            p1Denom += sum(trainMat[i])# 计算总共出现的不重复的单词数目
        else:
            p0Num += trainMat[i]
